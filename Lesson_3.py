...
# list_number = []
# while number != "qq":
#     list_number = input("Введите числа через пробел, я их сложу").split()
#     #на проверку число не число не делаем я про издиджитал
...
#     print(sum_number)
# print(f"Программа завершена, вввели qq. Сумма чисел равна {sum_number}")
...
def int_func(word):
    return word.title()
print(int_func("text"))
#Продолжаю работу над функцией, возможно я ее перепешу чуть, поэтому просто дублировал, чтобы понимали как я думаю
def int_func2(words):
    #words это теперь строка
    words_list = []
    words_list = words.split()
    result_list = []
    for word in words_list:
        result_list.append(word.title())
    my_string = ' '.join(result_list)
    return result_list, my_string
#Не совсем понял задание, поэтому сделал в виде списка и в виде строки.
print(int_func2("text text text text text"))
